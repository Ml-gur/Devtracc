-- DevTrack Africa - Complete Database Fix
-- This script ensures all tables, columns, triggers, and policies are properly set up
-- It handles both fresh installations and existing databases that need updates
...
-- Create or update projects table with all required columns
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'projects' AND column_name = 'progress_percentage') THEN
        ALTER TABLE public.projects ADD COLUMN progress_percentage INTEGER DEFAULT 0 CHECK (progress_percentage >= 0 AND progress_percentage <= 100);
    END IF;
...
CREATE TABLE IF NOT EXISTS public.posts (
    ...
    post_type TEXT NOT NULL DEFAULT 'text', -- 'text', 'project', 'progress', 'question', 'showcase', 'help', 'progress_update', 'task_completed', 'help_request'
    tags TEXT[],
    tech_stack TEXT[],
...
CREATE POLICY "Users can view own posts" ON public.posts FOR SELECT USING (user_id = auth.uid() OR author_id = auth.uid());
CREATE POLICY "Users can insert own posts" ON public.posts FOR INSERT WITH CHECK (user_id = auth.uid() OR author_id = auth.uid());
CREATE POLICY "Users can update own posts" ON public.posts FOR UPDATE USING (user_id = auth.uid() OR author_id = auth.uid());
CREATE POLICY "Users can delete own posts" ON public.posts FOR DELETE USING (user_id = auth.uid() OR author_id = auth.uid());

-- RLS Policies for comments table
CREATE POLICY "Users can view comments on accessible posts" ON public.comments FOR SELECT USING (
    post_id IN (
        SELECT id FROM public.posts 
        WHERE visibility = 'public' OR user_id = auth.uid() OR author_id = auth.uid()
    )
);
...
CREATE POLICY "Users can comment on projects" ON public.project_comments FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can view all project comments" ON public.project_comments FOR SELECT USING (true);
CREATE POLICY "Users can update their own project comments" ON public.project_comments FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "Users can delete their own project comments" ON public.project_comments FOR DELETE USING (auth.uid() = user_id);
...
CREATE POLICY "Users can unlike their own likes" ON public.project_likes FOR DELETE USING (auth.uid() = user_id);
...
CREATE OR REPLACE FUNCTION public.create_project(
    ...
    project_category TEXT DEFAULT 'other',
    project_is_public BOOLEAN DEFAULT true
)
RETURNS UUID AS $$
DECLARE
    new_project_id UUID;
BEGIN
    INSERT INTO public.projects (
        ...
        tags, tech_stack, visibility, category, is_public
    )
    VALUES (
        auth.uid(), project_title, project_description, project_status, 
        project_priority, project_start_date, project_end_date, 
        project_tags, project_tech_stack, project_visibility, project_category, project_is_public
    )
...
SELECT 'DevTrack Africa database completely fixed and ready! Auto-profile creation enabled, all tables and policies set up correctly.' AS status;
