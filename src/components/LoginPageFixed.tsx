...
export default function LoginPage({ onBack, onSuccess, onNeedConfirmation, onNavigateToRegister }: LoginPageProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!result.success) {
        if (result.error?.includes('Email not confirmed') || result.error?.includes('verify your email')) {
  ...
  const isAuthError = errors.general && (
    errors.general.includes('Invalid email or password') ||
    errors.general.includes('Invalid login credentials') ||
                      ...
                      <div className="mt-3 p-3 bg-blue-50 dark:bg-blue-900/20 rounded border-l-4 border-blue-400">
                        <div className="text-sm text-blue-800 dark:text-blue-200">
                          ...
                          <ul className="mt-1 ml-4 list-disc space-y-1">
                            <li>Try the demo access button below</li>
                            <li>Create a new account using "Sign up here" link</li>
                            <li>Double-check your email spelling and password</li>
